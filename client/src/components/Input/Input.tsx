...
export default function Input({ isSearch, setIsSearch }: InputProps) {
  const { inputValue, setInputValue, setCards } = useCards();
  ...
  const fetchSearchResults = async () => {
    try {
      const response = await fetch(
          `http://localhost/api/filter?prompt=${encodeURIComponent(inputValue)}`
      );
      if (!response.ok) {
      ...
      // ✅ Обновляем карточки через setCards
      setCards(data);
    } catch (error) {
